...
class PortsControllerTest < ActionController::TestCase
  ...
  test "should create port" do
    assert_difference('Port.count') do
      post :create, :port => { :city => @port.city, :country => @port.country, :ocean => @port.ocean, :port_name => @port.port_name, :state => @port.state }
    end
  ...
  test "should update port" do
    put :update, :id => @port, :port => { :city => @port.city, :country => @port.country, :ocean => @port.ocean, :port_name => @port.port_name, :state => @port.state }
    assert_redirected_to port_path(assigns(:port))
  end
